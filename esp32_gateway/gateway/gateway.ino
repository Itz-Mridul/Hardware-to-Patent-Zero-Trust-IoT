...
bool announceReconnectOnNextHeartbeat = false;
...
void networkTask(void* pvParameters) {
  ...
  for (;;) {
    ...
    if (!mqtt.connected()) {
      ...
      const char* connectionState = mqttEverConnected ? "RECONNECTED" : "BOOT";

      // ⭐ LWT: If ESP32 dies, Broker will publish a retained OFFLINE JSON payload.
      char willPayload[192];
      snprintf(
        willPayload,
        sizeof(willPayload),
        "{\"device_id\":\"%s\",\"status\":\"OFFLINE\",\"connection_state\":\"BROKER_WILL\"}",
        DEVICE_ID
      );

      if (!mqtt.connect(DEVICE_ID, "mailbox/status", 1, true, willPayload)) {
         vTaskDelay(2000 / portTICK_PERIOD_MS);
         continue; 
      }

      // Success! Publish ONLINE status with enough context for the Pi to open a grace period.
      StaticJsonDocument<192> statusDoc;
      statusDoc["device_id"] = DEVICE_ID;
      statusDoc["status"] = "ONLINE";
      statusDoc["connection_state"] = connectionState;
      statusDoc["rssi"] = WiFi.RSSI();
      statusDoc["uptime_ms"] = now;

      char statusBuffer[192];
      serializeJson(statusDoc, statusBuffer, sizeof(statusBuffer));
      mqtt.publish("mailbox/status", statusBuffer, true);
      
      // ⭐ RESET timing so the first packet after reconnect doesn't have a fake "attack" delay
      lastHeartbeatMs = millis(); 
      mqttEverConnected = true;
      announceReconnectOnNextHeartbeat = true;
    ...
    if (now - lastHeartbeatMs >= HEARTBEAT_INTERVAL_MS) {
      unsigned long interPacketDelay = (lastHeartbeatMs == 0) ? 0 : (now - lastHeartbeatMs);
      lastHeartbeatMs = now;
      ...
      if (announceReconnectOnNextHeartbeat) {
        doc["connection_state"] = pendingConnectionState;
        doc["fresh_connection"] = true;
        announceReconnectOnNextHeartbeat = false;
      }
      
      // ⭐ PATENT-CRITICAL: Inter-packet delay for ML fingerprinting
      doc["inter_packet_delay"] = interPacketDelay;
      ...
      // Also send to HTTP endpoint for ML verification
      HTTPClient http;
      ...
      if (httpCode == 200) {
          ...
          Serial.println("\n⛔ ML REJECTED - Possible spoofing detected!\n");
      } else if (httpCode > 0) {
          Serial.printf("\n⚠️ HTTP Error: %d\n", httpCode);
      }
